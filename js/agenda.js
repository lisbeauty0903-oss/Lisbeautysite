...
async function render(){
 let [i,f]=range(),pf=$("filtroProfissional").value,m=await serviceMap(),days=[];
 ...
 $("agendaGrid").innerHTML=`<div class="calendar-columns ${view==="dia"?"one-day":""}">${days.map(d=>{
  let k=ds(d),aa=A.filter(x=>ds(new Date(x.inicio))===k&&(!pf||x.profissional_id===pf)),bb=B.filter(x=>ds(new Date(x.inicio))===k&&(!pf||x.profissional_id===pf));
...
async function slots(pref=""){
 let pid=$("agendaProfissional").value,date=$("agendaData").value,{dur}=summary();if(!pid||!date||!dur){$("agendaHorario").innerHTML='<option value="">Selecione profissional, serviços e data</option>';return}
 let dow=new Date(`${date}T12:00:00`).getDay(),h=H.find(x=>x.profissional_id===pid&&Number(x.dia_semana)===dow);
 if(!h){$("agendaHorario").innerHTML='<option value="">Profissional não trabalha nesta data</option>';return}
 ...
 $("agendaHorario").innerHTML=out.length?'<option value="">Selecione</option>'+out.map(x=>`<option value="${x}">${x}</option>`).join(""):'<option value="">Nenhum horário disponível</option>';if(pref&&out.includes(pref))$("agendaHorario").value=pref;
}
function clearA(){$("agendamentoForm").reset();$("agendamentoId").value="";$("agendaStatus").value="agendado";$("agendaDesconto").value=0;$("agendaValorPago").value=0;$("agendaDuracao").value="0 min";$("agendaValorTotal").value="R$ 0,00";$("agendaTotalFinal").textContent="R$ 0,00";profServices();$("agendamentoMessage").textContent=""}
...
document.addEventListener("DOMContentLoaded",async()=>{try{await base();await events();$("novoAgendamentoBtn").onclick=()=>{clearA();$("agendaData").value=ds(new Date());openP("agendamentoPanel")};$("novoBloqueioBtn").onclick=()=>{$("bloqueioForm").reset();$("bloqueioData").value=ds(new Date());openP("bloqueioPanel")};$("cancelarAgendamentoBtn").onclick=()=>closeP("agendamentoPanel");$("cancelarBloqueioBtn").onclick=()=>closeP("bloqueioPanel");document.querySelectorAll("[data-close]").forEach(x=>x.onclick=()=>closeP(x.dataset.close));$("agendaProfissional").onchange=()=>{profServices();slots()};$("agendaData").onchange=()=>slots();$("agendaDesconto").oninput=summary;$("agendamentoForm").onsubmit=saveA;$("bloqueioForm").onsubmit=saveB;$("filtroProfissional").onchange=render;
 $("hojeBtn").onclick=async()=>{ref=new Date();await events()};$("anteriorBtn").onclick=async()=>{ref.setDate(ref.getDate()+(view==="dia"?-1:-7));await events()};$("proximoBtn").onclick=async()=>{ref.setDate(ref.getDate()+(view==="dia"?1:7));await events()};$("visaoDiaBtn").onclick=async()=>{view="dia";$("visaoDiaBtn").classList.add("active-view");$("visaoSemanaBtn").classList.remove("active-view");await events()};$("visaoSemanaBtn").onclick=async()=>{view="semana";$("visaoSemanaBtn").classList.add("active-view");$("visaoDiaBtn").classList.remove("active-view");await events()}}catch(e){console.error(e);$("agendaGrid").innerHTML=`<div class="empty-state">Erro ao carregar agenda: ${esc(e.message)}</div>`}});
